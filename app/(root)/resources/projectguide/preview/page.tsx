import PageHeader from "@/components/shared/PageHeader"
import PageNav from "@/components/shared/PageNav"
import FramedImage from "@/components/shared/FramedImage"

function PreviewPage() {
    return (
        <div className="lg:max-w-3xl">
            <PageHeader
                page='Preview'
                pageSubheading="Project Guide"
                title="Preview Project Specification"
                description="In this section, we will review the email that was sent, so as to complete your project. We still need to cover number 1, which is a video representation and number 3 as shown in the image below. We also need to fill out the feedback and status report forms which we will do in the next section."
            />

            <FramedImage alt="email-pic" src='/assets/images/email3.jpg' />

            <p className="font-light mt-10 text-[#777970] dark:text-[#9a9c8d]">
                1. For the video representation, a preview of how the video should be done and recorded is available via the link provided below. Please note,
                the sound of the video should be muted.
            </p>

            <ul className="mt-4">
                <li className="mb-2">
                    <a
                        href='https://utfs.io/f/b722ac01-2a25-4b6a-87b1-b635417ca487-ckhye7.mp4'
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[#4f46e5] hover:underline"
                    >
                        Video Representation of simplon beauty care
                    </a>
                </li>
            </ul>

            <p className="font-light mt-10 text-[#777970] dark:text-[#9a9c8d]">
                2. For the source and compiled code, please note the following:
            </p>

            <ol className="list-disc mt-2 font-light text-[#777970] dark:text-[#9a9c8d] ml-4">
                <li className="mb-3">
                    Source Code: This is the folder containing all the code you have written. It includes the human-readable files where your code is typed.
                </li>
                <li>
                    Compiled Code: This is the output you will need to generate based on the technologies you are using. It typically consists of machine-readable
                    files produced from compiling your source code.
                </li>
            </ol>

            <p className="mt-5 text-[#777970] dark:text-[#9a9c8d] font-light">
                If you need assistance with the source code, please reach out to our team for support.
            </p>

            <PageNav
                prev={{ href: "/resources/projectguide/documentation", label: "Documentation", shortLabel: "Docs" }}
                next={{ href: "/resources/projectguide/feedbackform", label: "Feedback Form" }}
            />
        </div>
    )
}

export default PreviewPage
