...
export function AccordionFaqs() {
    return (
        <div id="faqs" className="py-24 px-6 md:px-8 bg-[#131412] border-t border-[#26271f]">
                ...
                <div className="md:col-span-4">
                    <div className="md:sticky md:top-32">
                        ...
                        <Link href="mailto:appvarsitytech@gmail.com">
                            <p className="mt-6 text-sm text-[#9a9c8d]">
                                Have more questions? <span className="underline text-[#f4f2ec] font-medium">Contact us</span>
                            </p>
                        </Link>
                    </div>
                </div>
                    ...
                    <Accordion type="single" collapsible defaultValue={faqsOnHomepage[0]?.value} className="w-full flex flex-col gap-3">
                        {faqsOnHomepage.map((item) => (
                            <AccordionItem
                                key={item.value}
                                value={item.value}
                                className="border border-[#26271f] bg-[#1b1c18] px-6 rounded-[14px] data-[state=open]:border-[#4f46e5]/40 transition-all"
                            >
                                <AccordionTrigger className="no-underline hover:no-underline text-[#f4f2ec] font-medium text-sm tracking-tight py-5">
                                    {item.trigger}
                                </AccordionTrigger>
                                <AccordionContent className="text-[#9a9c8d] tracking-tight text-sm leading-relaxed">{item.content}</AccordionContent>
                            </AccordionItem>
                        ))}
                    </Accordion>
                </div>
